...
@section('page-subtitle', 'Formulir pembaruan data paket langganan jus mingguan')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-3xl shadow-card border border-gray-100 p-6 sm:p-8">
        
        <form action="{{ route('admin.paket-langganan.update', $paket->id_paket) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="total_pengiriman" id="total_pengiriman" 
                               value="{{ old('total_pengiriman', $paket->total_pengiriman) }}" 
                               placeholder="Misal: 6"
                ...
                <div>
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3"
                              ...
                              class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-primary focus:bg-white focus:ring-1 focus:ring-primary outline-none transition-all @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $paket->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        @foreach($menus as $menu)
                            <label class="flex items-center gap-2 cursor-pointer p-1.5 rounded-lg hover:bg-gray-100 transition-colors">
                                <input type="checkbox" name="allowed_menus[]" value="{{ $menu->id_menu }}"
                                       ...
                                       {{ is_array(old('allowed_menus', $selectedMenus)) && in_array($menu->id_menu, old('allowed_menus', $selectedMenus)) ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        <label class="inline-flex items-center gap-2.5 cursor-pointer mt-1">
                            <input type="checkbox" name="gratis_ongkir" value="1" 
                                   ...
                                   {{ old('gratis_ongkir', $paket->gratis_ongkir ? '1' : '0') == '1' ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center gap-4 mt-2">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="is_active" value="1" class="w-4 h-4 text-primary focus:ring-primary"
                                       {{ old('is_active', $paket->is_active ? '1' : '0') == '1' ? 'checked' : '' }}>
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="is_active" value="0" class="w-4 h-4 text-primary focus:ring-primary"
                                       {{ old('is_active', $paket->is_active ? '1' : '0') == '0' ? 'checked' : '' }}>
