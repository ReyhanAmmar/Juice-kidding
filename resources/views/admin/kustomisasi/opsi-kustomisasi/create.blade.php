...
@section('title', 'Tambah Opsi Kustomisasi')
@section('page-title', 'Opsi Kustomisasi')
@section('page-subtitle', 'Tambah opsi detail baru')
    ...
    <div class="bg-white rounded-2xl shadow-card border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50">
            <h2 class="text-lg font-black text-gray-900 flex items-center gap-2">
                ...
                Form Tambah Opsi Kustomisasi
            </h2>
        </div>

        <form action="{{ route('admin.opsi-kustomisasi.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="id_tipe_opsi" id="id_tipe_opsi"
                    ...
                    <option value="{{ $tipe->id_tipe_opsi }}">{{ $tipe->nama_tipe }} ({{ $tipe->pilih_banyak ? 'Multi-select' : 'Single-select' }})</option>
                    @endforeach
                </select>
                @error('id_tipe_opsi')
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="harga_tambahan" id="harga_tambahan"
                           value="{{ old('harga_tambahan', 0) }}"
                           min="0"
                           placeholder="0"
                           class="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-gray-200 bg-white
                                  focus:border-primary focus:ring-2 focus:ring-primary-light
